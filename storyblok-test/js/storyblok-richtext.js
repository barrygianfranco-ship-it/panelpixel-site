/* Panel Pixel — TEST LOCALE, fase 3 migrazione Storyblok.
   NON usato dal sito reale (vive solo in storyblok-test/). Fa due cose:
   ...
   1. fetchStoryblokArticles(): chiama la Content Delivery API di
      ...
      js/main.js (quello vero) già si aspetta da data/articles.json —
      vedi adaptStoryblokStory() più sotto per la mappatura campo per
      campo. Il campo "corpo" (richtext) viene già convertito in HTML
      qui dentro, così la copia di main.js in questa cartella deve
      cambiare solo una riga (renderMarkdownBody) rispetto all'originale.

   2. storyblokRichtextToHtml(doc): converte un documento richtext
      Storyblok nello STESSO HTML che produce oggi js/markdown-components.js
      per il markdown — stesse classi CSS (article-box, article-gallery,
      article-columns3, article-embed, article-footnotes...), quindi il
      CSS del sito (mai toccato) si applica identico.
   ...
   - Il nodo "image" nativo di richtext non ha un campo per "destra"/
     "sinistra" (quella scelta oggi vive nel prefisso dell'alt in
     markdown) — qui va sempre in variante centrata. Nessuno dei due
     articoli già migrati usa varianti flottanti, quindi non blocca
     questo test.
...
// Il token pubblico (Content Delivery API) NON è scritto qui: il browser
// lo legge a runtime da storyblok-test/token.local.txt, un file scritto
// dall'utente e MAI aggiunto a git (va cancellato a mano a fine test,
// stessa disciplina già seguita per il file del Management API token —
// qui deve stare dentro il repo solo perché il browser lo raggiunge via
// fetch(), non perché sia meno sensibile del percorso precedente).
let storyblokCdaTokenCache = null;
async function getStoryblokCdaToken() {
  if (storyblokCdaTokenCache) return storyblokCdaTokenCache;
  const res = await fetch("token.local.txt");
  if (!res.ok) {
    throw new Error(
      "storyblok-test/token.local.txt non trovato: scrivici dentro il token Content Delivery API prima di ricaricare la pagina."
    );
  }
  storyblokCdaTokenCache = (await res.text()).trim();
  return storyblokCdaTokenCache;
}
...
/* ---- Componenti annidati (nodo "blok"): stesso HTML/classi prodotte oggi
   da js/markdown-components.js per <!--box:...-->, <!--gallery:...-->,
...
function storyblokBlokToHtml(blokNode) {
  ...
  switch (comp.component) {
    case "box": {
      ...
      // stesso trattamento di oggi, paragrafi separati da riga vuota.
...
/* ---- Adatta una story Storyblok alla forma che js/main.js già si aspetta
   da un articolo di data/articles.json (vedi il piano concordato) ---- */
function adaptStoryblokStory(story) {
  ...
  return {
    ...
    // Già HTML pronto, non markdown: vedi renderMarkdownBody nella copia
    // locale di js/main.js in questa stessa cartella.
...
async function fetchStoryblokArticles() {
  const token = await getStoryblokCdaToken();
  const url = `https://api.storyblok.com/v2/cdn/stories?token=${encodeURIComponent(
    token
  )}&version=draft&content_type=articolo&per_page=100`;
